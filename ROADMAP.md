...
- **What:** The installed plugin is a **directory copy**, not this checkout: `installed_plugins.json` gives `installPath` as `~/.claude/plugins/cache/check-chat-marketplace/check-chat/0.2.0`, copied 08-14 19:39, while the marketplace source is `/home/daniel/src/check-chat`. Five commits have landed since, and the copy is provably behind — `CENSUS_POPULATION` is absent from it, so R-002's `--census` has never once run in the shipped tool. `/reload-plugins` does not close the gap; it re-reads the same copy, and `plugin.json` still says `0.2.0`, so `claude plugin update` may decline to re-snapshot at all. Fix it where a reader can see it: the tool reports the directory its own package was imported from, and the skill says so when that is not the tree the person is editing. Provenance from the filesystem, not from git — the package must stay installable standalone, so `__file__`'s resolved root is the fact, and a `.git` alongside it is a hint, never a requirement.
- **Why:** The rule is **measure the shipping function, never a model of it**, and for five days the shipping function was the copy while `.venv/bin/pytest` measured the repo. That is the same defect the rule was written against, wearing the one disguise it had left: the model of the function *was* the function, one tree over. It has already produced a wrong claim — the diagnostic that opened trap 8 read `checkchat/transcript.py:68` out of the checkout while **executing** the copy, and was right only because that line happened to be identical in both. A number taken through the installed tool and a test taken through the repo are not comparable today, and nothing in either output says which tree it came from. This is also the honest reading of R-008's provenance: a commit SHA beside an aggregate is worth little if the aggregate came from a tree the SHA does not describe.
- **Outcome:** every report and every `--emit` names the root it ran from, a stale install is legible from the output rather than from a filesystem investigation, and the dev loop that keeps the two in step is written down where a contributor will find it.
- **Blocked-by:** —
- **Enables:** R-008
